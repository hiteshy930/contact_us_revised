...
public class RequestsDao {
	...
	public int makeRequestArchived(int id) {
		...
		try {
			connection = DbConnection.getConnection();
			PreparedStatement statementQuery = connection.prepareStatement("update requests set status = 0 where id = ?");
	...
	public int makeRequestActive(int id) {
		...
		try {
			connection = DbConnection.getConnection();
			PreparedStatement statementQuery = connection.prepareStatement("update requests set status = 1 where id = ?");
			statementQuery.setInt(1,id);
			numberOfRowsAffected = statementQuery.executeUpdate();
